...
import java.util.*;
/**
 * this class creat a single object of type CaseInsensitive, builds the insensitive case inverted index and answer to a
 * given query.
 * @param - caseInsensitiveInvertedIndex - the single object created by the function getInstance.
 */
public class CaseInsensitive extends AbstractInvertedIndex {
    ...
    private static CaseInsensitive caseInsensitiveInvertedIndex;
    private CaseInsensitive(){}

    /**
     * this function creats a single object of type CaseInsensitive.
     *
     * @return caseInsensitiveInvertedIndex - an object of type CaseInsensitive.
     */ 

    public static CaseInsensitive getInstance(){
        if(caseInsensitiveInvertedIndex == null){
            caseInsensitiveInvertedIndex = new CaseInsensitive();
    ...
    /**
     * this function builds the insensitive case inverted index- all the words converted to lower case.
     *
    ...
    public TreeSet<String> runQuery(String query) {
        ...
        for(String queryOrOperator : Utils.splitBySpace(query)){
            if(!opertors.contains(queryOrOperator)){
                wordCount ++;
                stack.push(invertedIndexMap.get(queryOrOperator.toLowerCase()));
            }
            else{
